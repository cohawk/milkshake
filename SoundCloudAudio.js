var SoundCloudAudio = Class.extend({
    ...
    smjs.onload = function () {
      ...
      soundManager.onready(function () {
        ...
        jsonp.src =
          ...
          escape(audio.playlistURL) +
          "&client_id=" +
          audio.clientId +
          "&callback=milk.soundCloudJSONCallback";
  ...
  gotStreamURL: function (response) {
    var songs;
    if ("tracks" in response) songs = response.tracks;
    else if (0 in response) songs = response;
    else songs = [response];
    audio.songs = songs;
    ...
    for (var i = 0; i < songs.length; i++) {
      ...
      soundManager.createSound({
        ...
        onplay: function () {
          ...
          shaker.infoMessages["SoundCloud"] =
            "music courtesy of <a href='" +
            s.user.permalink_url +
            "'>" +
            s.user.username +
            "</a>" +
            " - <a href='" +
        ...
        onfinish: function () {
          ...
          if (audio.trackPos < audio.tracks.length - 1) audio.trackPos++;
